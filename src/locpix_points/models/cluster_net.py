...
from torch_geometric.nn import HeteroConv, conv
...
class ClusterEncoder(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = HeteroConv(
            {
                ("clusters", "near", "clusters"): conv.SAGEConv(
...
class ClusterNet(torch.nn.Module):
    def __init__(self, config):
        ...
        self.clusterencoder_0 = ClusterEncoder(
            config["ClusterEncoderChannels"][0], config["ClusterEncoderChannels"][1]
        )
        self.clusterencoder_1 = ClusterEncoder(
            config["ClusterEncoderChannels"][1], config["ClusterEncoderChannels"][2]
        )
        self.clusterencoder_2 = ClusterEncoder(
            config["ClusterEncoderChannels"][2], config["ClusterEncoderChannels"][3]
        )
        self.linear = Linear(
            config["ClusterEncoderChannels"][3], config["OutputChannels"]
        )
